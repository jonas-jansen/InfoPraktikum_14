...
public class regNextVisitor extends regularExpressionBaseVisitor<Boolean> {
    ...
    public Boolean visitExpr(regularExpressionParser.ExprContext ctx) { 
        boolean b = this.star && this.level == 0 && ctx.getParent().getChildCount() == 1
                && ctx.getChildCount() == 1 && ctx.getChild(0) instanceof regularExpressionParser.OrexpContext;
    ...
    public Boolean visitQues(regularExpressionParser.QuesContext ctx) { 
        boolean b = this.star && this.level == 0 && ctx.getParent().getChildCount() == 1
                && ctx.getChildCount() == 2 && ctx.getChild(0) instanceof regularExpressionParser.OrexpContext;
        if (!b) {
            this.level++;
        } 
        if (reverse){
